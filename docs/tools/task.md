...
| `model` | `string \| string[]` | No | Per-spawn model selector, or an ordered candidate list. Highest-priority model source for that spawn — above `task.agentModelOverrides` and agent frontmatter — while configured fallback stays intact: a `@role` selector keeps that role and its `retry.fallbackChains` chain, while a concrete selector keeps the ordinary auth/default fallback. Empty, whitespace-only, or zero-expansion selectors are rejected before dispatch. Availability is resolved during execution, so unavailable or uncredentialed candidates use the configured fallback path. Item field in batch shape, top-level in flat shape. |
