...
- Task can now choose a model with model: "provider/model" or an ordered list of models. In batch tasks, each task can choose its own model. Eval agent() and workpool() accept the same model option in both JavaScript and Python, and a pool applies its selector on each keep-alive worker's first turn. A per-spawn selector is the highest-priority model source for that spawn, above per-agent overrides and agent frontmatter, and it keeps the configured fallback machinery: a `@role` selector retains that role and its `retry.fallbackChains` chain, while a concrete selector keeps the ordinary auth/default fallback. Added clearer instructions, validation, and regression tests. ([#11604](https://github.com/can1357/oh-my-pi/pull/11604) by [@mabinogi80503](https://github.com/mabinogi80503))
